...
Processing order (per gesture repetition), applied at the original 5120 Hz
sampling rate before resampling:
    1. Bandpass filter 20–700 Hz            (Butterworth order 4, zero-phase)
    2. Notch filters at 30, 50, 60, 90, 150 Hz  (IIR notch, zero-phase)
    3. Resample to TARGET_LENGTH samples
    4. Per-channel z-score normalization

Steps 1–2 follow the putEMG paper (Kaczmarek et al., 2019):
    "raw sEMG data were filtered using a bandpass filter with cutoff
     frequencies of 20 and 700 Hz. Furthermore, to reduce interference
     generated by the mains grid and other equipment, an adaptive notch
     filter attenuating frequencies of 30, 50, 90, 60, and 150 Hz was applied."
...
NOTCH_Q          = 30                        # quality factor (bandwidth = freq / Q)
...
def notch_filter(gesture_data, freqs=NOTCH_FREQS, Q=NOTCH_Q, fs=ORIGINAL_FS):
    """
    Apply a cascade of zero-phase IIR notch filters to remove narrow-band
    interference at specified frequencies.

    Parameters
    ----------
    gesture_data : ndarray, shape (samples, 24)
    freqs        : list of float — centre frequencies to attenuate (Hz)
...
def preprocess_gesture(gesture_data, apply_bandpass=APPLY_BANDPASS,
    ...
    Order (all filtering at original fs before resampling):
        1. Bandpass 20–700 Hz
        2. Notch at 30, 50, 60, 90, 150 Hz
        3. Resample to target_length
        4. Per-channel z-score normalize

    Parameters
    ----------
    gesture_data    : ndarray, shape (samples, 24) — raw gesture at original fs
    apply_bandpass  : bool — applies both bandpass and notch when True
    apply_zscore    : bool
    target_length   : int
