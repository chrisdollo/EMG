...
## Key Differences from `generalizable/`

| | `within_subject/` | `generalizable/baseline_models/` |
|---|---|---|
...
```
within_subject/deep_learning_approach/
├── model/
...
Model definitions and data utilities are shared from `public/`:
- `public/deep_learning_models.py` — EEGNet, ShallowConvNet, DeepConvNet, CNN_LSTM, EMG_TCN
- `public/emg_loader.py` — `make_within_subject_loaders()`, `load_all_subjects()`
