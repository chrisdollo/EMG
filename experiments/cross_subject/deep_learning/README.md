...
```
experiments/cross_subject/deep_learning/
...
└── results/
    ├── summary.txt                # Plain-text summary of all results
    ├── results_log_EMG_TCN.txt    # Per-subject LOSO results — EMG_TCN (auto-updated)
    ├── results_log_EEGNet.txt     # Per-subject LOSO results — EEGNet
    ├── results_log_ShallowConvNet.txt
    └── bar_chart_emg_tcn_loso.png
```
...
```
Shared data loader: `src/emg_loader.py`  
Shared model definitions: `src/deep_learning_models.py`
...
Code: `src/deep_learning_models.py`
...
Full per-subject tables are in the `results/results_log_<MODEL_TYPE>.txt` files.
See `results/summary.txt` for a combined overview and key observations.
