...
def process_subject_file(input_path, output_path=None,
    ...
    # column index → gesture ID  (matches G1,G2,G3,G6,G7,G8,G9 column order)
    gesture_ids = [1, 2, 3, 6, 7, 8, 9]
    ...
    for row in range(n_rows):
        for col in range(n_cols):
            ...
            if cell is None or not isinstance(cell, np.ndarray) or cell.size == 0:
                continue
            processed = preprocess_gesture(
                cell, apply_bandpass=apply_bandpass,
                apply_zscore=apply_zscore, target_length=target_length
            )  # (target_length, 24)
            X_list.append(processed.T)       # → (24, target_length)
            y_list.append(gesture_ids[col])

    X = np.stack(X_list)   # (N, 24, target_length)
    y = np.array(y_list)   # (N,)

    print(f"  {len(X_list)} reps → X:{X.shape}  y:{y.shape}")
...
def batch_process_subjects(input_dir, output_dir,
    ...
    print(f"Found {len(mat_files)} subject file(s) in {os.path.basename(input_dir)}/")
    ...
    for i, filename in enumerate(mat_files, 1):
        # emg_gestures_04_NU_mat.mat → "04"
        parts = filename.split("_")
        sub_num = parts[2] if len(parts) > 2 else filename.replace(".mat", "")

        print(f"[{i}/{len(mat_files)}] {filename}")
        input_path  = os.path.join(input_dir,  filename)
        output_path = os.path.join(output_dir, f"emg_gestures_{sub_num}_U.npz")
...
def batch_process_split_dirs(input_root, output_root,
                             splits=("train", "eval"),
                             apply_bandpass=APPLY_BANDPASS, apply_zscore=APPLY_ZSCORE,
                             target_length=TARGET_LENGTH, mat_key=MAT_KEY):
    for split in splits:
